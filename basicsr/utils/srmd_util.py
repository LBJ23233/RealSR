...
class BatchRealKernel(object):
    def __init__(self, l=17):
        self.l = l
        self.kernel_dir = '/home/yangyuqiang/datasets/RealSR/RealSR_Raw/RAW_BLC_ECC_0217/RAW400WB/x2/train/kernel_center7/'
        self.kernel_pool = glob.glob(self.kernel_dir + '*mat')
        np.random.shuffle(self.kernel_pool)
        self.len = len(self.kernel_pool)
        self.ext = '_kernel_x2.mat'

    def __call__(self, batch, path, tensor=False):
        batch_kernel = np.zeros((batch, self.l, self.l))
        for i in range(batch):
            if self.len == 0:
                self.kernel_pool = glob.glob(self.kernel_dir + '*mat')
                np.random.shuffle(self.kernel_pool)
                self.len = len(self.kernel_pool)
            name = path[i].split('/')[-1][:-5]
            # data = sio.loadmat(self.kernel_dir + name + self.ext)
            idx = np.random.randint(self.len)
            data = sio.loadmat(self.kernel_pool.pop(idx))
            self.len -= 1
            kernel = data['Kernel']
            kernel /= kernel.sum()
            # weight kernel
            # idx2 = np.random.randint(self.len)
            # data2 = sio.loadmat(self.kernel_pool[idx2])
            # k2 = data2['Kernel']
            # k2 /= k2.sum()
            # w = np.random.rand() / 2 + 0.5
            # kernel = kernel * w + k2 * (1 - w)
...
class BatchBlur(nn.Module):
    ...
    def forward(self, input, kernel):
        ...
        if len(kernel.size()) == 2:
            ...
            out = F.conv2d(input_CBHW, kernel_var, padding=0).view((B, C, H, W))
        else:
            ...
            out = F.conv2d(input_CBHW, kernel_var, groups=B*C).view((B, C, H, W))
        return F.avg_pool2d(out, kernel_size=2)
        # return out
...
class SRMDPreprocessing(object):
    def __init__(self, scale, pca, random, para_input=10, kernel=21, noise=True, cuda=False, sig=2.6, sig_min=0.2, sig_max=4.0, rate_iso=1.0, scaling=3, rate_cln=0.2, noise_high=0.08):
        ...
        self.kernel_gen = BatchRealKernel()
    ...
    def __call__(self, hr_tensor, hr_path, kernel=False):
        ...
        b_kernels = Variable(self.kernel_gen(B, hr_path, tensor=True)).cuda() if self.cuda else Variable(self.kernel_gen(B, hr_path, tensor=True))
        # blur
        if self.cuda:
            lr_blured_t = self.blur(Variable(hr_tensor).cuda(), b_kernels)
        else:
            lr_blured_t = self.blur(Variable(hr_tensor), b_kernels)
        # kernel encode
        ...
        return (lr_blured_t, b_kernels) if kernel else lr_blured_t
