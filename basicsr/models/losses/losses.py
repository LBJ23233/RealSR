...
from basicsr.models.archs.PyNET_arch import PyNET
...
class Raw2rgbLoss(nn.Module):
    def __init__(self, loss_weight=1.0, criterion='l1'):
        super(Raw2rgbLoss, self).__init__()
        self.loss_weight = loss_weight
        self.isp = PyNET(level=0, instance_norm=True, instance_norm_level_1=True)
        self.isp.load_state_dict(torch.load("/home/yangyuqiang/tmp/PyNET-PyTorch/models/pynet_level_0_epoch_499.pth"), strict=False)
        print('load PyNET done!')
        self.isp.eval()
    ...
    def forward(self, sr, gt):
        ...
        gt_rgb = self.isp(gt)
        raw2rgb_loss = 0
        raw2rgb_loss += self.criterion(sr_rgb, gt_rgb)
        return raw2rgb_loss * self.loss_weight
